...
<div id="infoCont"></div>
	...
	var pronos =  ol.proj.get('EPSG:32721');
		...
		window.loadFeaturesz = function (response) {
			 sourcezWFS.addFeatures(new ol.format.GeoJSON().readFeatures(response));
		};
 ...
 /*   map.addInteraction(interactionSelect);
    ...
    function iniciarestados(){
        ...
        for(var j = 0; j < contens.length; j++){
            ...
            switch (cnv)  {
            case 0:
				cn.setStyle(new ol.style.Style({
        ...
        for(var h = 0; h < zonet.length; h++){
        	...
        	for(var i = 0; i < contens.length; i++){
        		var cn = contens[i];
                var cnv = cn.get('zona_idzona');
                if (cnv==znv[1]){ //El contenedor esta en la zona
						num_cont++;
						if(cn.get('cestado') == 0 || cn.get('cestado') == 1){
								num_cont_des++;
							}
                    }
    ...
    switch ($(this).attr('id')) {
        ...
        case 'btnCho':
            ...
            //todo           
                ...
                var spl = e.target.item(0).getId().split(".");
                if(spl[0] == 'zona'){
                	...
                	info.innerHTML = e.target.item(0).getId() + ' Estado: ' + tipore + ' Tipo: ' + tiporee;
                }
            });
            map.addInteraction(interaction);
            break;
